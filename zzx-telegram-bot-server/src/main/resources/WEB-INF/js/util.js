...
function getMessage2(accid, msg) {
	var str = '';
	var attach = null,url = '';
	if(msg.attach != '' && null != msg.attach) {
		try {
			attach = $.parseJSON(msg.attach);
			url = attach.name ? _$escape(attach.url) : '';
		} catch (err) {
			
		}
	}
	var sentStr = (accid === msg.sendId) ? "发送" : "收到";
	switch (msg.msgType) {
	case 'TEXT':
	...
	case 'FILE':
		if(msg.msgStatus === 2) {
			...
			if (/png|jpg|bmp|jpeg|gif/i.test(attach.ext)) {
                 if (url) {
                    url = nim.viewImageSync({
                        url: url
                    })
            ...
            } else if (!/exe|bat/i.test(attach.ext)) {
                if (attach.name) {
                    url += attach.url.indexOf('?') === -1 ? '?' : '&'
                    url += 'download=' + encodeURI(_$escape(attach.name));
                }
                str = '<a href="' + url + '" target="_blank" class="download-file f-maxWid"><span class="icon icon-file2"></span>' + _$escape(attach.name) + '</a>';
            } else {
                str = '<p>[非法文件，已被本站拦截]</p>';
            }
		}
...
function buildSender2(accid, msg) {
	if(accid === msg.sendId) {
		return 'me';
	}
	return 'you';
...
function transNotification(item) {
    ...
    switch (type) {
        ...
        case 'removeTeamManagers':
            ...
            break
        case 'leaveTeam':
            var member = (item.from === userUID) ? item.fromNick : getNick(item.from);
            str = member + "&nbsp;&nbsp;结束了会话";
            return str;
            break;
        case 'updateTeam':
...
function updateWaiterStatus(waiterId, status) {
	$.ajax({
		type: 'POST',
		async: false,
		url: '/updateWaiterStatus',
		dataType: 'json',
		data: {waiterId: waiterId, status: status},
		success: function(result) {
			console.log(waiterId + (status == 1 ? '上线' : '离线') + '成功');
		},
		error: function(xhr, options, error) {
			console.log(waiterId + (status == 1 ? '上线' : '离线') + '失败');
		}
  });
}

function logoutWeb(waiterId) {
	$.ajax({
		type: 'POST',
		async: false,
		url: '/logout',
		...
		success: function(result) {
			console.log(waiterId + '离线' + '成功');
		},
		error: function(xhr, options, error) {
			console.log(waiterId + '离线' + '失败');
		}
  });
}

function getProductLink(productId, skuId, shopId, usingAttributes, url) {
	  if(usingAttributes == 4) {
		  return '<a class="pdhover" href="http://192.168.89.38:8080/package/skus.htm?skuId=' + skuId + '&shopId=' + shopId
		     + '" target="_blank"><img width="100" height="100" src="' + url + '"/></a>';
	  }
	  return '<a class="pdhover" href="http://192.168.89.38/product/productInstruction.htm?id='+ productId
	     + '" target="_blank"><img width="100" height="100" src="' + url + '"/></a>';
}

function buildCustomRecommendProduceMsgHtml(message, customAttach, user) {
...
function buildCustomPushOrderItemMsgHtml(message, user) {
    let content = JSON.parse(message.content.replace(/'/g,"\""));
    let type = content.attach.type;
    let attach = content.attach;
    let userShow = content.userShow;
...
//    console.log("userShow : " + JSON.stringify(content));
    ...
    if(type === "pushOrderItem") {
        msgHtml = [
            ...
            '<div class="cnt">' +
            '<table style="border-collapse:collapse;word-wrap:break-word;word-break:break-all;">' +
            '<tr><td colspan="2">订单编号['+attach.orderId+']</td></tr>' +
            '<tr><td width="100" rowspan="3">' + getOrderLink(attach.orderId, attach.icon) +
            '</td><td width="200" style="padding:5px 0px 5px;">' + (attach.productName.length > 20 ? (attach.productName.substring(0, 20) + '...') : attach.productName) +
            '</td></tr>' +
            '<tr><td style="padding:5px 0px 5px;">￥' + (parseInt(attach.price) / 100) +
            '</td></tr>' +
            '</table>' +
            '</div>' +
            '</div>' +
...
function buildCustomOrderMsgHtml(message, user) {
    let content = JSON.parse(message.content.replace(/'/g,"\""));
    let attach = content.attach;
...
function getOrderLink(orderId, url) {
    // return '<a class="pdhover" href="http://192.168.89.38:8080/package/skus.htm?skuId=' + skuId + '&shopId=' + shopId
    //         + '" target="_blank"><img width="100" height="100" src="' + url + '"/></a>';

    return '<a class="pdhover" href="http://192.168.89.38:8080/package/skus.htm?skuId=' + orderId +' target="_blank">' +
...
/**
 * 查看浏览器是否支持DOM Storage
 */
function isSupportDomStorage() {
    //sessionStorage
