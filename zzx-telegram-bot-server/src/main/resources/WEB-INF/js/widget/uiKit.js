...
/***/ function(module, exports, __webpack_require__) {
		...
		util.addEvent(this._body,'click',function(e){
	        ...
	        while(self!==target){
	        	if (target.tagName.toLowerCase() === "img") {
	                ...
	                var teamName = item.getAttribute("data-tname");
	                cbClickPortrait(account,type, teamName);
	                return;
	            }else if(target.tagName.toLowerCase() === "li"){
	        	 	account = target.getAttribute("data-account");
	                type = target.getAttribute("data-type");
	                var teamName = target.getAttribute("data-tname");
	                util.removeClass(util.getNode(".j-team li.active",that._body),'active');
	                util.addClass(target,"active");
	                cbClickList(account,type, teamName);
	                return;
	            }
	            target = target.parentNode;
	        }    
	...
	TeamList.prototype.update2 = function(list){
		console.log('---update2---crtSession:' + this.crtSession);
		var tmHtml = '',info,tmp1 = '<div class="panel_team"><ul class="j-normalTeam">';
		if(list && list.length > 0) {
			for (var i = 0, len = list.length; i < len; ++i) {
				info = list[i];
				tmHtml += ['<li class="panel_item" data-gtype="normal" data-tname="' + info.teamName + '" data-type="team" data-account="' + info.teamId + '">',
                         '<div class="panel_avatar"><img class="panel_image" src="images/normal.png"/></div>',
                         '<div class="panel_text">',
                             '<p class="panel_single-row">'+info.teamName+'</p>',
                         '</div>',
                     '</li>'].join("");
			}
			tmHtml = tmp1 + tmHtml + '</ul></div>';
		} else {
			tmHtml = '<p>暂时还没有接受用户咨询哦</p>';
		}
		this._body.innerHTML = tmHtml;
	};
	/**
	 * 更新视图
	 * @param  {Object} data 
