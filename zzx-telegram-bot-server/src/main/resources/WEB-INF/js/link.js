...
var SDKBridge = function(ctr, data) {
  ...
  // 加载敏感词库
  loadSensitiveWordToCache(this);
  // 加载知识库
  loadKnowledgeToCache(this);
   ...
   * 加载自定义敏感词库
   */
  function loadSensitiveWordToCache(t) {
    $.ajax({
      url: '/sensitiveWord/1.0.0/selectAll',
      type: 'POST',
      data: {
      },
      success: function(result) {
        if(result.code == 0) {
          let sensitiveWord = result.data;
          if (sensitiveWord != null && sensitiveWord.length > 0) {
            let stList = t.cache.getSensitiveWord();
            if (stList != null && stList.length > 0) {
              t.cache.clearSensitiveWord();
            }
            t.cache.addSensitiveWord(sensitiveWord);
            console.log("加载成功的敏感词库：" + t.cache.getSensitiveWord());
            t.cache.makeSensitiveWordMap(sensitiveWord);
            console.log("构建敏感词结构："+JSON.stringify(t.cache.getSensitiveWordMap()));
          }
        } else {
          console.log("加载敏感词库失败");
        }
      },
      error: function(xhr, options, error) {
        console.log("加载敏感词库异常");
      }
    });
  }
   ...
   * 加载自定义知识库
   */
  function loadKnowledgeToCache(t) {
    $.ajax({
      url: '/knowledgeBase/1.0.0/selectAll',
      type: 'POST',
      data: {
      ...
      success: function(result) {
        if(result.code == 0) {
          let knowledge = result.data;
          let stList = t.cache.getKnowledgeBase();
          if (stList != null && stList.length > 0) {
             t.cache.clearKnowledgeBase();
          }
          t.cache.addKnowledgeBase(knowledge);
          t.cache.makeKnowledgeBaseMap(knowledge);
          console.log("加载成功的知识库：" + t.cache.getKnowledgeBase());
        } else {
          console.log("加载知识库失败");
        }
      },
