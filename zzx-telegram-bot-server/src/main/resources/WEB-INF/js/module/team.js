...
YX.fn.team = function () {
	// 左侧面板群组中的三个按钮
	$('#transferChatBtn').on('click', this.showTeamView.bind(this, 0, 2));
}
/**
 * 最近联系人显示
 * @return {void}
...
YX.fn.buildTeams = function (id) {
	var that = this;
	$.post('/getHistoryChatList', {waiterId: waiterId}, function (result) {
		if(result.code != 0) {
			console.log('获取历史会话列表失败');
			return;
		}
		var options = {
			data: result.data,
			infoprovider: that.infoProvider.bind(that),
			onclickitem: that.openChatBox2.bind(that)
		}
		that.teams = new NIMUIKit.TeamList(options);
		that.teams.inject($('#teams').get(0));
		that.doPoint();
	});
	/*var data = {
...
YX.fn.showTeamView = function (type, teamType) {
	...
	if(teamType == 0 || teamType == 1) {
		...
		$.post('/getWaiterList', function (result) {
			if(result.code != 0) {
				alert(result.msg);
				return;
			}
			that.dialog.open2({list: result.data, cbConfirm: that.transferWaiter, env: that, yx: that});
			that.addTeamMemberTag = false;
		...
		$.post('/getWaitingList', {waiterId: waiterId}, function (result) {
			if(result.code != 0) {
				alert(result.msg);
				return;
			}
			that.dialog.open3({list: result.data, cbConfirm: that.takeWaiter, env: that, yx: that});
			that.addTeamMemberTag = false;
		}).error(function (xhr, status, info) {
			that.dialog.close();
			alert('获取排队用户异常');
		...
		$.post('/getPharmacyList', function (result) {
			if(result.code != 0) {
				alert(result.msg);
				return;
			}
			that.dialog.open4({list: result.data, cbConfirm: that.recommendProduct, env: that, yx: that});
			that.addTeamMemberTag = false;
		}).error(function (xhr, status, info) {
			that.dialog.close();
			alert('获取药店列表异常');
...
YX.fn.recommendProduct = function() {
	var goodsId = $('input:radio[name="r_recommend_goods_id"]:checked').val();
	if(!goodsId) {
		alert('请选择您将要推荐的药品');
		return;
	}
...
YX.fn.takeWaiter = function() {
	var that = this;
	var userId = $('input:radio[name="r_take_user_id"]:checked').val();
	if(!userId) {
		alert('请选择您将要接待的用户');
		return;
	}
	$.post('/takeWaiter', {waiterId: waiterId, userId: userId}, function (result) {
		if(result.code != 0) {
			alert(result.msg);
			return;
		}
...
YX.fn.transferWaiter = function() {
	var that = this;
	var toWaiterId = $('input:radio[name="r_transfer_waiter"]:checked').val();
	if(!toWaiterId) {
		alert('请选择您将要转接的客服');
		return;
	}
