...
YX.fn.initModule = function() {
  this.initDistributionList(null);
  this.initBase();
  this.message();
  this.notification();
  this.personCard();
  ...
  this.pollingWaitingCount();
};

YX.fn.updateDistributionList = function(list, teamId) {
	distributionList = new Array();
	if(null != list && list.length > 0) {
		...
		for(var i = 0; i < list.length; i++) {
			team = list[i];
			session = new Object();
			session.id = 'team-' + team.teamId;
			session.scene = 'team';
			session.to = team.teamId;
			session.time = team.createTime;
			session.unread = 0;
			distributionList.push(session);
		}
	}
...
YX.fn.initDistributionList = function(msg) {
	var that = this;
	$.ajax({
		url: '/getSessionList',
		type: 'POST',
			...
			if(null != msg) {
				...
				switch (type) {
					...
					case 'removeTeamMembers':
						that.buildSessions();
						that.$chatContent.removeClass('chat-content-bottom').addClass('msg-content-bottom');
						$('#chatEditor').addClass('hide');
						$('#endChatBtn').addClass('hide');
						$('#transferChatBtn').addClass('hide');
						$('#recommendProductBtn').addClass('hide');
						break;
				}
			}
		},
		error: function(xhr, options, error) {
			console.log('获取会话列表异常');
		}
	});
}

YX.fn.pollingWaitingCount = function() {
	var that = this;
	setInterval(function() {
		that.getWaitingCount();
	}, 60000);
}

YX.fn.getWaitingCount = function() {
	$.post('/getWaitingCount', {waiterId: waiterId}, function (result) {
		if(result.code != 0) {
			return;
		}
		var $waitingCount = $('#waiting_count');
		var count = result.data;
		if($waitingCount.hasClass('hide')) {
			if(count > 0) {
				$waitingCount.removeClass('hide').text(count);
			}
		} else {
			if(count > 0) {
				$waitingCount.text(count);
			} else {
				$waitingCount.addClass('hide').text('');
			}
		}
	});
}
...
YX.fn.initBase = function() {
  ...
  this.$showNotice.on('click', this.showTeamView.bind(this, 0, 3));
  $('#recommendProductBtn').on('click', this.showTeamView.bind(this, 0, 4));
  //切换面板
...
YX.fn.openChatBox2 = function(account, scene, teamName) {
	...
	this.$chatContent.removeClass('chat-content-bottom').addClass('msg-content-bottom');
	$('#chatEditor').addClass('hide');
	$('#endChatBtn').addClass('hide');
	$('#transferChatBtn').addClass('hide');
	$('#recommendProductBtn').addClass('hide');
	this.$chatTitle.find('img').attr('src', 'images/normal.png');
    this.$nickName.text(teamName);
    this.doPoint();
    this.$chatContent.html('');
    this.getChatMessageList(account,1,10);
}
...
YX.fn.openChatBox = function(account, scene) {
  ...
  this.$rightPanel.find('.chat-mask').addClass('hide');
  this.$rightPanel.removeClass('hide');
  this.$chatContent.removeClass('msg-content-bottom').addClass('chat-content-bottom');
  $('#chatEditor').removeClass('hide');
  this.$messageText.val('');
  $('#showTeamMsgReceipt').hide();
  var $endChatBtn = $('#endChatBtn');
  var $transferChatBtn = $('#transferChatBtn');
  var $recommendProductBtn = $('#recommendProductBtn');
  ...
  if (scene == 'p2p') {
    ...
    $transferChatBtn.addClass('hide');
    $recommendProductBtn.addClass('hide');
    this.$messageText.removeClass('p2p').addClass('p2p2');
  } else {
    //群聊
    info = this.cache.getTeamById(account);
    this.$teamInfo && this.$teamInfo.removeClass('hide');
    $endChatBtn.removeClass('hide');
    $transferChatBtn.removeClass('hide');
    $recommendProductBtn.removeClass('hide');
    this.$messageText.removeClass('p2p2').addClass('p2p');
    if (info) {
      if (info.avatar) {
        this.$chatTitle
          .find('img')
  ...
  this.doPoint();
  // 根据或取聊天记录
  this.getHistoryMsgs2(scene, account);
};
...
YX.fn.infoProvider = function(data, type) {
  ...
  switch (type) {
    case 'session':
      ...
      info.scene = scene;
      if(!msg && scene === 'team') {
    	  info.account = data.to;
    	  info.target = scene + '-' + data.to;
    	  info.time = transTime2(data.time);
    	  info.text = '';
      } else {
      ...
      if (scene === 'p2p') {
        ...
        //群组
        var teamInfo = this.cache.getTeamById(info.account);
        if (teamInfo) {
