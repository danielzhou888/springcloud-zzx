	...
	var action = {
		...
		render:function(res){
			laytpl($("#template").html()).render(res.data,function(html){
				$("#sensitive-word-container").html(html);
				...
				$(".row-del").on("click",function(event){
					...
					layer.confirm('是否确认删除？', {
						...
						api.delSensitiveWord({id:id},function(res){
							if(res.code == 0){
								layer.alert("删除成功", {
									icon: 1,
									time: 1000,
		...
		callback:function(params){
			api.showSensitiveWord(params,function(res){
					console.log(res);
					if(res.code == 0){
