...
layui.define(['common','api','laytpl','laypage'],function(exports){
	...
	var action = {
			...
			laytpl($("#template").html()).render(res.data,function(html){
				...
				$(".row-del").on("click",function(event){
					...
					layer.confirm('是否确认删除？', {
					  ...
					  api.delUser({id:id},function(res){
					  	if(res.code == 0){
							layer.msg("删除成功");
							location.reload();
						} else {
				...
				$(".addBlackBtn").on("click", function (event) {
					let id = $(event.target).attr("data");
					action.addBlackEvent(id);
				})
			});
		},
		callback:function(params){
			api.showUser(params,function(res){
				console.log(res);
				if(res.code == 0){
					action.render(res);
				} else {
		...
		addBlackEvent:function (id) {
			console.log("addBlackEvent id = "+id);
			var params = {id:id};
			api.addToBlackByUserId(params, function (res) {
				if (res.code == 0) {
					layer.alert(res.msg, {
						icon: 1,
						time: 1000,
