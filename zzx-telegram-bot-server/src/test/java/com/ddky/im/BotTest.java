...
public class BotTest {
    ...
    public static final Logger logger = LoggerFactory.getLogger(BotTest.class);
    ...
    public void main(String[] args) {
        List<String> adminBots = Arrays.asList(AdminBots);
        List<String> commonBots = Arrays.asList(CommonBots);

        int adminBotCount = adminBots.size();
        int commonBotCount = commonBots.size();
        ...
        for (int i = 0; i < adminBotCount; i++) {
            ...
            for (int j = 0; j < 3; j++) {
                new Thread(new Runnable() {
                    ...
                    public void run() {
                        while (true) {
                            String msg = getRandomAdminMessage();
                            msg.replaceAll(PROJECT_NAME_TAG, PROJECTNAME);
                            String reqUrl = adminUrl + msg;
